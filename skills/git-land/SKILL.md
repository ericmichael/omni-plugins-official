...
2. Confirm the full check suite is green locally before any push.
3. If the working tree has uncommitted changes, commit with the `git-commit` skill
   and push with the `git-push` skill before proceeding.
4. Check mergeability and conflicts against main.
5. If conflicts exist, use the `git-pull` skill to fetch/merge `origin/main` and
   resolve conflicts, then use the `git-push` skill to publish the updated branch.
6. Ensure review comments (if present) are acknowledged and any required
   fixes are handled before merging.
7. Watch checks until complete.
8. If checks fail, pull logs, fix the issue, commit with the `git-commit` skill,
   push with the `git-push` skill, and re-run checks.
...
if [ "$mergeable" = "CONFLICTING" ]; then
  # Run the `git-pull` skill to handle fetch + merge + conflict resolution.
  # Then run the `git-push` skill to publish the updated branch.
fi
...
# Wait for review feedback: reviews arrive as PR comments. Treat them as
# feedback that must be acknowledged before merge.
while true; do
  gh api repos/{owner}/{repo}/pulls/"$pr_number"/comments \
    --jq '.[] | .id' | head -1 | rg -q '.' && break
  sleep 10
done
...
```
python3 .omni_code/skills/git-land/land_watch.py
```
...
- If checks fail, pull details with `gh pr checks` and `gh run view --log`, then
  fix locally, commit with the `git-commit` skill, push with the `git-push` skill, and
  re-run the watch.
...
- Do not enable auto-merge; use the watch loop to control the merge.
...
- If feedback requires changes:
  - Reply with intended fixes (`[agent] ...`) **as an inline reply to the
    original review comment** using the review comment endpoint and
...
- If review feedback expands scope, decide whether to include it now or defer
  ...
  call it out in the root-level update with a brief reason (e.g.,
