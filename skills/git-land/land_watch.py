...
Exit codes:
  0: All checks passed and no outstanding review comments.
  2: Review comments detected — address feedback before merge.
  3: CI checks failed.
  4: PR head updated externally — pull and retrigger CI.
  5: PR has merge conflicts — resolve before running land_watch again.
"""
import asyncio
import json
import random
...
# Bot users whose comments are treated as automated review feedback.
# Add your CI bots, code review bots, etc. here.
AGENT_BOTS: set[str] = {
    "github-actions[bot]",
}
MAX_GH_RETRIES = 5
...
def filter_human_review_comments(
    ...
    for comment in comments:
        ...
        last_reply = None
        if root is not None:
            last_reply = latest_agent_reply.get(root)
...
def is_blocking_review(review: dict[str, Any]) -> bool:
    created_at = review.get("submitted_at") or review.get("created_at")
    if not created_at:
        return False
    user_login = review.get("user", {}).get("login")
...
def filter_blocking_reviews(
    ...
    return [review for review in dedupe_reviews(reviews) if is_blocking_review(review)]
...
def raise_on_human_feedback(
    ...
    human_issue = filter_human_issue_comments(issue_comments)
    human_review = filter_human_review_comments(review_comments)
    if human_issue or human_review:
        ...
        raise SystemExit(2)
    blocking = filter_blocking_reviews(reviews)
    if blocking:
...
async def wait_for_reviews(pr_number: int, checks_done: asyncio.Event) -> None:
...
async def watch_pr() -> None:
    ...
    review_task = asyncio.create_task(wait_for_reviews(pr.number, checks_done))
